...
import { getAutoResponse } from '../config/autoResponses.js';
...
async function handleAutoResponse(message, client) {
  try {
    ...
    if (!content) {
      await message.reply('yo you pinged me but said nothing 👀 try saying something!');
      return;
    }
    ...
    logger.error('Error in handleAutoResponse:', error);
  }
}
